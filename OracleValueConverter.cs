...
namespace Uzex.ORM.Dapper.Oracle
{
  ...
  {
    private static string[] OracleStringTypes { get; } =
    {
      "Oracle.DataAccess.Types.OracleString",
      "Oracle.ManagedDataAccess.Types.OracleString"
    };
    ...
    public static T Convert<T>(object val)
    {
      ...
      if (val == DBNull.Value)
      {
        if (default(T) != null) throw new ApplicationException("Attempting to cast a DBNull to a non nullable type!");
      ...
      if (Regex.IsMatch(valueType.FullName, @"Oracle\.\w+\.Types\.Oracle\w+"))
      {
        ...
        {
          var reflected = valueProperty.GetValue(val);
          return (T) reflected;
        }
      }
